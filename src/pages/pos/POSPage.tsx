...
import { computeOrderTotals, paymentsMatchTotal } from './calc';
...
export function POSPage() {
  ...
  if (!currentStore || !currentBranch) {
    return (
      <div>
        ...
        <EmptyState message="Выберите магазин и филиал в шапке, чтобы начать продажу." />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader title="Касса" subtitle={`${currentStore.name} — ${currentBranch.name}`} />
      <div className={styles.grid}>
        ...
        <div className={styles.right}>
          ...
          <Card>
            ...
            >
              {checkoutMutation.isPending ? 'Проводим…' : 'Оплатить'}
            </Button>
          </Card>
        </div>
      </div>
